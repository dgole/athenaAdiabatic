        ...
        for (k=pG->ks; k<=pG->ke; k++) {
          for (j=pG->js; j<=pG->je; j++) {
            for (i=pG->is; i<=pG->ie; i++) {
	    	  ...
	    	  press = pG->U[k][j][i].E - 0.5*(SQR(pG->U[k][j][i].M1)
                            + SQR(pG->U[k][j][i].M2)
                            + SQR(pG->U[k][j][i].M3))/pG->U[k][j][i].d;
			  press -= 0.5*(SQR(pG->U[k][j][i].B1c)
                            + SQR(pG->U[k][j][i].B2c)
                            + SQR(pG->U[k][j][i].B3c));
			  press *= Gamma_1;
...
/* If there's inflow into the grid, set the normal velocity to zero */
       ...
       pG->U[ks-k][j][i].E += 0.5*(SQR(pG->U[ks-k][j][i].B1c)
		...
		if (pG->U[ks-k][j][i].B1c*pG->U[ks-k][j][i].B1c + pG->U[ks-k][j][i].B2c*pG->U[ks-k][j][i].B2c + pG->U[ks-k][j][i].B3c*pG->U[ks-k][j][i].B3c > maxB) {
		maxB = pG->U[ks-k][j][i].B1c*pG->U[ks-k][j][i].B1c + pG->U[ks-k][j][i].B2c*pG->U[ks-k][j][i].B2c + pG->U[ks-k][j][i].B3c*pG->U[ks-k][j][i].B3c;
        maxBi = i; maxBj = j; maxBk = ks-k;}

        // look for large B values
		if (abs(pG->U[ks-k][j][i].B1c) + abs(pG->U[ks-k][j][i].B2c) + abs(pG->U[ks-k][j][i].B3c) > 1.0) {
     	  printf("%s %.2i %.2i %.2i %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G \n", "LARGE B FOUND in gz ", i, j, ks-k, pG->U[ks-k][j][i].E, 
		  pG->U[ks-k][j][i].M1, pG->U[ks-k][j][i].M2, pG->U[ks-k][j][i].M3, pG->U[ks-k][j][i].d, pG->U[ks-k][j][i].B1c, 
		  pG->U[ks-k][j][i].B2c, pG->U[ks-k][j][i].B3c, pG->U[ks][j][i].B1c, pG->U[ks][j][i].B2c, pG->U[ks][j][i].B3c, pressks);		   	
		}

		// look for small pressure in pressks
		if (pressks<=TINY_NUMBER) {
     	  printf("%s %.2i %.2i %.2i %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G \n", "TINY NUMBER FOUND in pressks ", i, j, ks-k, pG->U[ks-k][j][i].E, 
		  pG->U[ks-k][j][i].M1, pG->U[ks-k][j][i].M2, pG->U[ks-k][j][i].M3, pG->U[ks-k][j][i].d, pG->U[ks-k][j][i].B1c, 
		  pG->U[ks-k][j][i].B2c, pG->U[ks-k][j][i].B3c, pG->U[ks][j][i].B1c, pG->U[ks][j][i].B2c, pG->U[ks][j][i].B3c, pressks);	
		}


/*		if (pG->U[ks-k][j][i].d != pG->U[ks-k][j][i].d) {printf("%s %i %i %i \n", "NAN FOUND IN d_gz   at ", i, j, k);}
		if (pG->U[ks-k][j][i].M1 != pG->U[ks-k][j][i].M1) {printf("%s %i %i %i \n", "NAN FOUND IN M1_gz   at ", i, j, k);}
		if (pG->U[ks-k][j][i].M2 != pG->U[ks-k][j][i].M2) {printf("%s %i %i %i \n", "NAN FOUND IN M2_gz   at ", i, j, k);}
		if (pG->U[ks-k][j][i].M3 != pG->U[ks-k][j][i].M3) {printf("%s %i %i %i \n", "NAN FOUND IN M3_gz   at ", i, j, k);}
		if (pG->B1i[ks-k][j][i] != pG->B1i[ks-k][j][i]) {printf("%s %i %i %i \n", "NAN FOUND IN B1i_gz   at ", i, j, k);}
		if (pG->B2i[ks-k][j][i] != pG->B2i[ks-k][j][i]) {printf("%s %i %i %i \n", "NAN FOUND IN B2i_gz   at ", i, j, k);}
		if (pG->B3i[ks-k][j][i] != pG->B3i[ks-k][j][i]) {printf("%s %i %i %i \n", "NAN FOUND IN B3i_gz   at ", i, j, k);}
		if (pG->U[ks-k][j][i].B1c != pG->U[ks-k][j][i].B1c) {printf("%s %i %i %i \n", "NAN FOUND IN B1_gz   at ", i, j, k);}
		if (pG->U[ks-k][j][i].B2c != pG->U[ks-k][j][i].B2c) {printf("%s %i %i %i \n", "NAN FOUND IN B2_gz   at ", i, j, k);}
		if (pG->U[ks-k][j][i].B3c != pG->U[ks-k][j][i].B3c) {printf("%s %i %i %i \n", "NAN FOUND IN B3_gz   at ", i, j, k);}
		if (pG->U[ks-k][j][i].E != pG->U[ks-k][j][i].E) {printf("%s %i %i %i \n", "NAN FOUND IN E_gz   at ", i, j, k);}
*/
  ...
  printf("%s %.2i %.2i %.2i %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G \n", "LARGEST B IN GZs: ", maxBi, maxBj, maxBk, pG->U[maxBk][maxBj][maxBi].E, 
  pG->U[maxBk][maxBj][maxBi].M1, pG->U[maxBk][maxBj][maxBi].M2, pG->U[maxBk][maxBj][maxBi].M3, pG->U[maxBk][maxBj][maxBi].d, pG->U[maxBk][maxBj][maxBi].B1c, 
  pG->U[maxBk][maxBj][maxBi].B2c, pG->U[maxBk][maxBj][maxBi].B3c);
...
/* If there's inflow into the grid, set the normal velocity to zero */
       ...
       pG->U[ke+k][j][i].E += 0.5*(SQR(pG->U[ke+k][j][i].B1c)
		...
		// find the largest B in the gzs
		if (pG->U[ke+k][j][i].B1c*pG->U[ke+k][j][i].B1c + pG->U[ke+k][j][i].B2c*pG->U[ke+k][j][i].B2c + pG->U[ke+k][j][i].B3c*pG->U[ke+k][j][i].B3c > maxB) {
		maxB = pG->U[ke+k][j][i].B1c*pG->U[ke+k][j][i].B1c + pG->U[ke+k][j][i].B2c*pG->U[ke+k][j][i].B2c + pG->U[ke+k][j][i].B3c*pG->U[ke+k][j][i].B3c;
        maxBi = i; maxBj = j; maxBk = ke+k;}

        // look for large B values
		if (abs(pG->U[ke+k][j][i].B1c) + abs(pG->U[ke+k][j][i].B2c) + abs(pG->U[ke+k][j][i].B3c) > 1.0) {
     	  printf("%s %.2i %.2i %.2i %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G  \n", "LARGE B FOUND in gz ", i, j, ke+k, pG->U[ke+k][j][i].E, 
		  pG->U[ke+k][j][i].M1, pG->U[ke+k][j][i].M2, pG->U[ke+k][j][i].M3, pG->U[ke+k][j][i].d, pG->U[ke+k][j][i].B1c, 
		  pG->U[ke+k][j][i].B2c, pG->U[ke+k][j][i].B3c, pG->U[ke][j][i].B1c, pG->U[ke][j][i].B2c, pG->U[ke][j][i].B3c, presske);		   	
		}

		// look for small pressure in presske
		if (presske<=TINY_NUMBER) {
     	  printf("%s %.2i %.2i %.2i %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G %.9G  \n", "TINY NUMBER FOUND in presske ", i, j, ke+k, pG->U[ke+k][j][i].E, 
		  pG->U[ke+k][j][i].M1, pG->U[ke+k][j][i].M2, pG->U[ke+k][j][i].M3, pG->U[ke+k][j][i].d, pG->U[ke+k][j][i].B1c, 
		  pG->U[ke+k][j][i].B2c, pG->U[ke+k][j][i].B3c, pG->U[ke][j][i].B1c, pG->U[ke][j][i].B2c, pG->U[ke][j][i].B3c, presske);	
		}

/*		if (pG->U[ke+k][j][i].d != pG->U[ke+k][j][i].d) {printf("%s %i %i %i \n", "NAN FOUND IN d_gz   at ", i, j, k);}
		if (pG->U[ke+k][j][i].M1 != pG->U[ke+k][j][i].M1) {printf("%s %i %i %i \n", "NAN FOUND IN M1_gz   at ", i, j, k);}
		if (pG->U[ke+k][j][i].M2 != pG->U[ke+k][j][i].M2) {printf("%s %i %i %i \n", "NAN FOUND IN M2_gz   at ", i, j, k);}
		if (pG->U[ke+k][j][i].M3 != pG->U[ke+k][j][i].M3) {printf("%s %i %i %i \n", "NAN FOUND IN M3_gz   at ", i, j, k);}
		if (pG->B1i[ke+k][j][i] != pG->B1i[ke+k][j][i]) {printf("%s %i %i %i \n", "NAN FOUND IN B1i_gz   at ", i, j, k);}
		if (pG->B2i[ke+k][j][i] != pG->B2i[ke+k][j][i]) {printf("%s %i %i %i \n", "NAN FOUND IN B2i_gz   at ", i, j, k);}
		if (pG->B3i[ke+k][j][i] != pG->B3i[ke+k][j][i]) {printf("%s %i %i %i \n", "NAN FOUND IN B3i_gz   at ", i, j, k);}
		if (pG->U[ke+k][j][i].B1c != pG->U[ke+k][j][i].B1c) {printf("%s %i %i %i \n", "NAN FOUND IN B1_gz   at ", i, j, k);}
		if (pG->U[ke+k][j][i].B2c != pG->U[ke+k][j][i].B2c) {printf("%s %i %i %i \n", "NAN FOUND IN B2_gz   at ", i, j, k);}
		if (pG->U[ke+k][j][i].B3c != pG->U[ke+k][j][i].B3c) {printf("%s %i %i %i \n", "NAN FOUND IN B3_gz   at ", i, j, k);}
		if (pG->U[ke+k][j][i].E != pG->U[ke+k][j][i].E) {printf("%s %i %i %i \n", "NAN FOUND IN E_gz   at ", i, j, k);}
*/
