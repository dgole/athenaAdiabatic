        ...
        for (k=pG->ks; k<=pG->ke; k++) {
          for (j=pG->js; j<=pG->je; j++) {
            for (i=pG->is; i<=pG->ie; i++) {
			  ...
			  //if (press < P_FLOOR ){printf("%s %.9G %i %i %i %i \n",     "Pressure floor applied ", press, i, j, k, nd);}
		...
		for (k=pG->ks-nghost; k<=pG->ks; k++) {
		  for (j=pG->js-nghost; j<=pG->je+nghost; j++) {
			for (i=pG->is-nghost; i<=pG->ie+nghost; i++) {
			  Real tempPres=pG->U[k][j][i].E;
			  tempPres-=0.5*pG->U[k][j][i].B1c*pG->U[k][j][i].B1c;
 			  tempPres-=0.5*pG->U[k][j][i].B2c*pG->U[k][j][i].B2c;
			  tempPres-=0.5*pG->U[k][j][i].B3c*pG->U[k][j][i].B3c;
			  tempPres-=0.5*pG->U[k][j][i].M1*pG->U[k][j][i].M1/pG->U[k][j][i].d;
			  tempPres-=0.5*pG->U[k][j][i].M2*pG->U[k][j][i].M2/pG->U[k][j][i].d;
   			  tempPres-=0.5*pG->U[k][j][i].M3*pG->U[k][j][i].M3/pG->U[k][j][i].d;
			  tempPres*=Gamma_1;			  
			  if (tempPres<lowestPres1){lowestPres1=tempPres; lowestPresi1=i; lowestPresj1=j; lowestPresk1=k;}
            }}}
  }}}
  printf("%s %.9G %i %i %i \n", "Lowest Pressure in GZs1: ", lowestPres1, lowestPresi1, lowestPresj1, lowestPresk1);
		  ...
		  for (j=pG->js-nghost; j<=pG->je+nghost; j++) {
			for (i=pG->is-nghost; i<=pG->ie+nghost; i++) {
			  ...
			  tempPres-=0.5*pG->U[k][j][i].M2*pG->U[k][j][i].M2/pG->U[k][j][i].d;
   			  tempPres-=0.5*pG->U[k][j][i].M3*pG->U[k][j][i].M3/pG->U[k][j][i].d;
			  tempPres*=Gamma_1;			  
			  if (tempPres<lowestPres2){lowestPres2=tempPres; lowestPresi2=i; lowestPresj2=j; lowestPresk2=k;}
            }}}
  }}}
  printf("%s %.9G %i %i %i \n", "Lowest Pressure in GZs2: ", lowestPres2, lowestPresi2, lowestPresj2, lowestPresk2);
		  ...
		  for (j=pG->js; j<=pG->je; j++) {
			for (i=pG->is; i<=pG->ie; i++) {
			  ...
			  tempPres-=0.5*pG->U[k][j][i].M2*pG->U[k][j][i].M2/pG->U[k][j][i].d;
   			  tempPres-=0.5*pG->U[k][j][i].M3*pG->U[k][j][i].M3/pG->U[k][j][i].d;
			  tempPres*=Gamma_1;			  
			  if (tempPres<lowestPres3){lowestPres3=tempPres; lowestPresi3=i; lowestPresj3=j; lowestPresk3=k;}
            }}}
  }}}
  printf("%s %.9G %i %i %i \n", "Lowest Pressure in box: ", lowestPres3, lowestPresi3, lowestPresj3, lowestPresk3);
...
/* If there's inflow into the grid, set the normal velocity to zero */
       ...
       pG->U[ks-k][j][i].E += 0.5*(SQR(pG->U[ks-k][j][i].B1c)
         + SQR(pG->U[ks-k][j][i].B2c) + (pG->U[ks-k][j][i].B3c));
#endif /* MHD */
#endif /* ADIABATIC */
...
// CHECKS AND SUCH, DOESN'T ACTUALLY DO ANYTING CODE-WISE
		...
		Real tempPress=Gamma_1*(pG->U[ks-k][j][i].E 
					   -0.5*(SQR(pG->U[ks-k][j][i].M1)+SQR(pG->U[ks-k][j][i].M2)+SQR(pG->U[ks-k][j][i].M3))/pG->U[ks-k][j][i].d
					   -0.5*(SQR(pG->U[ks-k][j][i].B1c)+SQR(pG->U[ks-k][j][i].B2c)+(pG->U[ks-k][j][i].B3c)));
		printf("%s %.9G \n", "Bottom GZs", press/tempPress);
...
/* If there's inflow into the grid, set the normal velocity to zero */
       ...
       pG->U[ke+k][j][i].E += 0.5*(SQR(pG->U[ke+k][j][i].B1c)
		...
		Real tempPress=Gamma_1*(pG->U[ke+k][j][i].E - 0.5*(SQR(pG->U[ke+k][j][i].M1) + SQR(pG->U[ke+k][j][i].M2) + SQR(pG->U[ke+k][j][i].M3))/pG->U[ke+k][j][i].d - 0.5*(SQR(pG->U[ke+k][j][i].B1c) + SQR(pG->U[ke+k][j][i].B2c) + (pG->U[ke+k][j][i].B3c)));
		printf("%s %.9G \n", "Top GZs", press/tempPress);
		...
		if (presske<=TINY_NUMBER) {
     	  printf("%s %.2i %.2i %.2i %.9G %.9G %.9G  \n", "TINY NUMBER FOUND in presske ", i, j, ke+k,
		  0.5*(SQR(pG->U[ke+k][j][i].B1c)+SQR(pG->U[ke+k][j][i].B2c)+SQR(pG->U[ke+k][j][i].B3c))/pG->U[ke+k][j][i].E,
		  0.5*(SQR(pG->U[ke+k][j][i].M1)+SQR(pG->U[ke+k][j][i].M2)+SQR(pG->U[ke+k][j][i].M3))/pG->U[ke+k][j][i].E,
		  0.5*((SQR(pG->U[ke+k][j][i].M1)+SQR(pG->U[ke+k][j][i].M2)+SQR(pG->U[ke+k][j][i].M3))+(SQR(pG->U[ke+k][j][i].B1c)+SQR(pG->U[ke+k][j][i].B2c)+SQR(pG->U[ke+k][j][i].B3c)))/pG->U[ke+k][j][i].E);	
		}
      }
    }
  }
