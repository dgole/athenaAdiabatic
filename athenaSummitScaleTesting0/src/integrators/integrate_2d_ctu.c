 ...
 */
    ...
    if (CoolingFunc != NULL){
      for (i=il+1; i<=iu; i++) {
        coolfl = (*CoolingFunc)(Wl[i].d,Wl[i].P,(0.5*pG->dt), pG, i, j, 0);
        coolfr = (*CoolingFunc)(Wr[i].d,Wr[i].P,(0.5*pG->dt), pG, i, j, 0);
 ...
 */
    ...
    if (CoolingFunc != NULL){
      for (j=jl+1; j<=ju; j++) {
        coolfl = (*CoolingFunc)(Wl[j].d,Wl[j].P,(0.5*pG->dt), pG, i, j, 0);
        coolfr = (*CoolingFunc)(Wr[j].d,Wr[j].P,(0.5*pG->dt), pG, i, j, 0);
 ...
 */
  ...
  for (j=jl+1; j<=ju; j++) {
    for (i=il+1; i<=iu-1; i++) {
      ...
      Ur_x2Face[j][i].Mz += pG->dt*Om*pG->U[ks][j][i].M2;
      Ur_x2Face[j][i].Mx += hdt*(qshear-2.0)*Om*pG->U[ks][j][i].M1;

      Ul_x2Face[j][i].Mz += pG->dt*Om*pG->U[ks][j-1][i].M2;
      Ul_x2Face[j][i].Mx += hdt*(qshear-2.0)*Om*pG->U[ks][j-1][i].M1;
    }
  }
#endif
 ...
 */
  ...
  if (CoolingFunc != NULL){
    for (j=js; j<=je; j++){
      for (i=is; i<=ie; i++){
        coolf = (*CoolingFunc)(dhalf[j][i],phalf[j][i],pG->dt, pG, i, j, 0);
