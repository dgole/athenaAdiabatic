...
// calculate density at for each z and store in pGrid->U[k][j][i].d with j=js-1, i=is-1
  j=js-1; i=is-1;
  pGrid->U[(ke+1+nghost)/2][j][i].d  = den;
  pGrid->U[(ke+1+nghost)/2][j][i].B1c = Tmid;
  for (k=(ke+1+nghost)/2+1; k<=ke; k++) {
    // calculate T0 for previous cell
	cc_pos(pGrid,i,j,k-1,&x1,&x2,&x3);
	if (fabs(x3)<zq) {
	  T0nm1=Tatm+(Tmid-Tatm)*pow(cos(PI*x3/(2.0*zq)),2.0*delta);}
	else {
	  T0nm1=Tatm;}
    // calculate T0 for this cell
	cc_pos(pGrid,i,j,k,&x1,&x2,&x3);
	if (fabs(x3)<zq) {
	  T0n=Tatm+(Tmid-Tatm)*pow(cos(PI*x3/(2.0*zq)),2.0*delta);}
	else {
	  T0n=Tatm;}
	pGrid->U[k][j][i].d = pGrid->U[k-1][j][i].d*(2.0 - ((x3-pGrid->dx3)*pGrid->dx3/T0nm1) - (T0n/T0nm1));
	pGrid->U[k][j][i].B1c = T0n; // store temp as B1c in a gz for now 
    //printf("%i %i %i %0.9G %0.9G %0.9G %0.9G \n", i, j, k, x3, pGrid->U[k][j][i].d, T0n, T0nm1);
  }


  // PASTE MATHEMATICA OUTPUT HERE

	pGrid->U[24][js-1][is-1].d = 1.34645; 
	pGrid->U[25][js-1][is-1].d = 1.15792; 
	pGrid->U[26][js-1][is-1].d = 0.877359; 
	pGrid->U[27][js-1][is-1].d = 0.606704; 
	pGrid->U[28][js-1][is-1].d = 0.395272; 
	pGrid->U[29][js-1][is-1].d = 0.247841; 
	pGrid->U[30][js-1][is-1].d = 0.151085; 
	pGrid->U[31][js-1][is-1].d = 0.0896698; 
	pGrid->U[32][js-1][is-1].d = 0.0515805; 
	pGrid->U[33][js-1][is-1].d = 0.028534; 
	pGrid->U[34][js-1][is-1].d = 0.0150491; 
	pGrid->U[35][js-1][is-1].d = 0.00751029; 
	pGrid->U[36][js-1][is-1].d = 0.00352876; 
	pGrid->U[37][js-1][is-1].d = 0.00155755; 
	pGrid->U[38][js-1][is-1].d = 0.000645559; 
	pGrid->U[39][js-1][is-1].d = 0.000251255; 
	pGrid->U[40][js-1][is-1].d = 0.0000918266; 
	pGrid->U[41][js-1][is-1].d = 0.0000315139; 
	pGrid->U[42][js-1][is-1].d = 0.0000101555; 
	pGrid->U[43][js-1][is-1].d = 0.0000030729; 




// do the -z half of the box
  j=js-1; i=is-1;
  pGrid->U[(ke+1+nghost)/2][j][i].d  = den;
  for (k=(ke+1+nghost)/2; k<=ke; k++) {
    int ktemp = (ke+nghost) - k;
	pGrid->U[ktemp][j][i].d = pGrid->U[k][j][i].d;
	pGrid->U[ktemp][j][i].B1c = pGrid->U[k][j][i].B1c; // store temp as B1c in a gz for now 
    //printf("%i %i %i %0.9G %0.9G %0.9G %0.9G \n", i, j, k, x3, pGrid->U[k][j][i].d, T0n, T0nm1);
  }

// print out density values at each z if wanted
  //printf("%s \n", "MARKER 1");
  j=js-1; i=is-1;
  for (k=ks; k<=ke; k++) {
	cc_pos(pGrid,i,j,k,&x1,&x2,&x3);
    //printf("%i %i %i %0.9G %0.9G \n", i, j, k, x3, pGrid->U[k][j][i].d);
  }
  //printf("%s \n", "MARKER 2");
 ...
 */
      if (ipert == 1) {
		...
		//rd = den*exp(-x3*x3)*(1.0+2.0*rval); // original routine
        rd = pGrid->U[k][js-1][is-1].d*(1.0+2.0*rval); // my routine
        if (rd < D_FLOOR) {rd = D_FLOOR;}
		#ifdef ADIABATIC
		  //rp = pres/den*rd;  // original routine
	      rp = rd*pGrid->U[k][js-1][is-1].B1c;  // my routine
		  if (rp < P_FLOOR) {rp = P_FLOOR;}
		#endif
		//printf("%i %i %i %0.9G %0.9G %0.9G %0.9G  %0.9G \n", i, j, k, x1, x2, x3, pGrid->U[k][j][i].d, rp);
...
{
//  printf("%s \n", "marker at start of get_eta_user routine");
  int k0,rj,zi;
  Real x3;
...
/* Here is our equation for Am in the FUV active layer */
  ...
  //Am_FUV = 2.36e16*(ionfrac_FUV/1.e-5)*pow(R_AU,3./2.)*myrho;
...
static void density_profile(MeshS *pM)
{
  printf("%s \n", "MARKER 1 inside density_profile");  
  GridS *pG;
  DomainS *pD;
  int i,j,k,kg,kdisp;
  double darea,Lx,Ly;
...
#endif
  printf("%s \n", "MARKER 2 inside density_profile"); 
/* At level=0, there is only one domain */
  ...
  if (pG != NULL) {
    ...
    kdisp=pG->Disp[2];
  printf("%s \n", "MARKER 3 inside density_profile"); 
/* Compute 1d averaged density */
  ...
  printf("%s \n", "MARKER 4 inside density_profile"); 
    Lx = pM->RootMaxX[0] - pM->RootMinX[0];
    Ly = pM->RootMaxX[1] - pM->RootMinX[1];
    darea = (double)(pG->dx1*pG->dx2/(Lx*Ly));
  printf("%s \n", "MARKER 5 inside density_profile"); 
    for (k=0; k<pM->Nx[2]; k++) {
      rho_avg[k] *= darea;
    }
  printf("%s \n", "MARKER 6 inside density_profile"); 
...
{
  printf("%s \n", "MARKER 1 inside Userwork_before_loop");
  GridS *pG;
  DomainS *pD;
  printf("%s \n", "MARKER 2 inside Userwork_before_loop");
/* At level=0, there is only one domain */
  pG = pM->Domain[0][0].Grid;
  if (pG != NULL) {
    pD = (DomainS*)&(pM->Domain[0][0]);
  }
  printf("%s \n", "MARKER 3 inside Userwork_before_loop");
  nxgrid = pD->NGrid[0];
  nygrid = pD->NGrid[1];
  nzgrid = pD->NGrid[2];
  ...
  printf("%s \n", "MARKER 4 inside Userwork_before_loop");
  ...
  printf("%s \n", "MARKER 5 inside Userwork_before_loop");
#ifdef RESISTIVITY
  ...
  char line[MAXLEN];
  FILE *fp;
  printf("%s \n", "MARKER 6 inside Userwork_before_loop");
...
/* calculate the horizontally averaged density profile */
    density_profile(pM);
    printf("%s \n", "MARKER 8 inside Userwork_before_loop");
