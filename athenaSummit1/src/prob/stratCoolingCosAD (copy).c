...
#endif
  Omega_0 = par_getd_def("problem","omega",1.0e-3); // in restart
  qshear  = par_getd_def("problem","qshear",1.5);   // in restart
  amp = par_getd("problem","amp");    // not in restart
  beta = par_getd("problem","beta");  // not in restart
  pres = par_getd("problem","pres");  // not in restart 
  B0 = sqrt((double)(2.0*pres/beta)); // not in restart
  ifield = par_geti_def("problem","ifield", 1); // not in restart
  ipert = par_geti_def("problem","ipert", 1);   // not in restart
  R_AU = par_getd("problem","R_AU");    // in restart   
  Am0 = par_getd("problem","Am0");      // in restart
  Sig_FUV = par_getd("problem","Sig_FUV"); // in restart
  ionfrac_FUV = par_getd("problem","ionfrac_FUV"); // in restart

#ifdef ADIABATIC // all in restart
