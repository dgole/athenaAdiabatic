			...
			for (i=pG->is-nghost; i<=pG->ie+nghost; i++) {
		      Real tempPres=Gamma_1*(pG->U[k][j][i].E 
								...
								- 0.5*(SQR(pG->U[k][j][i].B1c) 
						             + SQR(pG->U[k][j][i].B2c) 
                                     + SQR(pG->U[k][j][i].B3c)));
		      Real tempB2=		  0.5*(SQR(pG->U[k][j][i].B1c) 
						             + SQR(pG->U[k][j][i].B2c) 
                                     + SQR(pG->U[k][j][i].B3c));
		      Real tempBeta=tempPres/tempB2;	  
			  if (tempBeta<lowestPres4){lowestPres4=tempBeta; lowestPresnd4=nd; lowestPresi4=i; lowestPresj4=j; lowestPresk4=k;} 
            }}}
  }}}
  printf("%s %.9G %i %i %i %i \n", "Lowest Beta in entire box: ", lowestPres4, lowestPresnd4, lowestPresi4, lowestPresj4, lowestPresk4);
...
/* Copy the velocities, but not the momenta --- important because of the density extrapolation above */
        //pG->U[ks-k][j][i].M1 = pG->U[ks][j][i].M1;
        //pG->U[ks-k][j][i].M2 = pG->U[ks][j][i].M2;
...
/* Copy the velocities, but not the momenta --- important because of the density extrapolation above */
        //pG->U[ke+k][j][i].M1 = pG->U[ke][j][i].M1;
        //pG->U[ke+k][j][i].M2 = pG->U[ke][j][i].M2;
