...
/* At level=0, there is only one domain */
  ...
  for (k=ks;k<=ke;k++){
    if (Sigma_top[k] >= Sig_FUV && check_bot == 1) {
      zib = zbtm + ((Real)(k-nghost)+0.5)*pM->dx[2];
      check_bot = 0;
    }
    if (Sigma_bot[k] <= Sig_FUV && check_top == 1) {
...
/* Determine indices corresponding to zib and zit */
  for (k=0;k<=pM->Nx[2];k++){
    z = pM->RootMinX[2] + (k + 0.5)*pG->dx3;
    if (z > -zib-delz && z <= -zib-delz+pG->dx3) kbase1 = k+k/pG->Nx[2]*2*nghost+nghost;
    if (z < zit+delz && z >= zit+delz-pG->dx3) kbase2 = k+k/pG->Nx[2]*2*nghost+nghost;
  }
...
/* X-ray ionization */
    ...
    ion_rate[k] += 2.0*(Lx/1.0e29)*pow(R_AU,-2.2)
           *(r1*(exp(-pow(nh1/(col1),pow1)) + exp(-pow(nh2/(col1),pow1)))
           + r2*(exp(-pow(nh1/(col2*0.2),pow2)) + exp(-pow(nh2/(col2*0.2),pow2))));
...
/* Combine densities from all grids */
/* for use in FUV transition calculation */
