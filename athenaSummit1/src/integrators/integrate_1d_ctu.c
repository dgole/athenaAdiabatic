 ...
 */
  ...
  if (CoolingFunc != NULL){
    for (i=il+1; i<=iu; i++) {
      coolfl = (*CoolingFunc)(Wl[i].d,Wl[i].P,(0.5*pG->dt), pG, i, 0, 0);
      coolfr = (*CoolingFunc)(Wr[i].d,Wr[i].P,(0.5*pG->dt), pG, i, 0, 0);
 ...
 */
  ...
  if (CoolingFunc != NULL){
    for (i=is; i<=ie; i++) {
      coolf = (*CoolingFunc)(dhalf[i],phalf[i],pG->dt, pG, i, 0, 0);
